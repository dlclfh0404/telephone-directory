...
def print_menu(): # 출력되는 부분
    print()
    print("="*50)
    print("1. 조회, 2. 입력, 3. 수정, 4. 삭제, 5. 종료")
    print("="*50)

def phonebook_search(): # 조회 기능
    global phonebook
    print("1. 조회")
    a = input("조회할 이름 또는 전화번호를 입력하세요 : ").strip() # strip으로 문자열 및 공백 제거
    idx = 0 
    for i in phonebook:
        if a == "": 
...
def phonebook_insert(): # 입력 기능
    global phonebook
    print("2. 입력")
    a = input("입력할 이름을 입력하세요 : ").strip()
    b = input("입력할 전화번호를 입력하세요 : ").strip()
    
    if a == "" or b == "": # 입력값이 없으면 다시 입력 화면 return
...
def phonebook_update(): # 수정 기능
    global phonebook
    print("3. 수정")
    a = input("이름을 입력하세요 : ").strip()
    b = input("변경할 전화번호를 입력하세요 : ").strip()
    
    if a == "" or b == "": # 입력값이 없으면 다시 수정 화면 return
        print("입력값이 부족합니다. 다시 입력해주세요.")
        phonebook_update()
        return
